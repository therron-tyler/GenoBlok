...
### `bulk/02_normalize_cluster/PROMOTER_bulk_pipeline_20260527_WCGNA.R` 🟦
...
### `bulk/03_robustness/PROMOTER_cluster_robustness_V1V2.R` 🟦
...
- `regression_qst_v1.py` — nested-CV regression zoo (OLS/Ridge/Lasso/ElasticNet/SVR/kNN)
...
- `cluster_sweep_adj_sex.py` — per-cluster effect on the phenotype adjusted for age + sex
...
### `singlecell/01_build_object/Processing_HashReps_into_MergedObj.R` 🟦
...
- `IMPACT_normalization_variant_pipeline.R` — runs **merged / SCTv2 / RPCA / Harmony**
  ...
  (e.g. `"1:9,11:12;1:12"`).
- `NPSLE_Tcell_SCTransformV2_Pipeline.R` — focused SCTransform-v2-only path
...
- `NPSLE_PCA_SCT_Investigation_Pipeline.R` — which genes drive each PC **before vs after**
...
- `IMP12_SCTv2_Investigation_Pipeline.R` — single-sample/batch deep-dive: SCT, Harmony,
...
- `NPSLE_Tcell_Subclustering_Pipeline.R` — subcluster (CD8/Memory-CD4/Naive-CD4),
  compare groups via **propeller** + **MASC (lme4)** proportion testing + pseudobulk DESeq2.
- `NPSLE_Tcell_Subcluster_Survey.R` — per-dims survey PDFs: clustree, DE heatmaps,
  canonical violins/feature plots, ADT panels.
- `SingleCell_RDS_list_DimRezScanner_UMAP.R` — scan dims × resolution across an RDS list.
- `NPSLE_ADT_UMAP_Survey_Pipeline.R` — ADT/RNA UMAP survey, one page per ADT marker.

### `singlecell/05_figures_report/IMPACT_one_shot_figure_pipeline_pval_v5-2.R` 🟦
...
- **Out:** `Figures/*.png`, `Tables/*.csv`, `IMPACT_auto_filled.pptx`.
...
### `report/decks/` 🐍
- `build_npsle_pca_sct_deck.py` — assemble a captioned PowerPoint from a figure folder
...
### `report/apps/fate_mapping_correct_formatting.R` 🟦
